...
use quote::quote;
use syn::{parse_macro_input, LitStr, Token, punctuated::Punctuated, Ident, Expr};
...
fn load_config() -> LocalizeConfig {
    ...
    if config_path.exists() {
        let content = fs::read_to_string(config_path)
            .expect("Failed to read localize.toml");
        toml::from_str(&content)
            .expect("Failed to parse localize.toml")
    } else {
...
impl syn::parse::Parse for LocalizeArgs {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        if input.peek(Token![,]) {
            ...
            let punctuated: Punctuated<syn::Expr, Token![,]> =
